...
{
    public sealed class KTMSExceptionHandler(
    ILogger<KTMSExceptionHandler> logger,
...
) : IExceptionHandler
    {
        public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
        {
            ...
            {
                KTMSNotFoundException => StatusCodes.Status404NotFound,
                KTMSConflictException or KTMSBusinessRuleException => StatusCodes.Status409Conflict,
        ...
        private static ErrorDto BuildErrorDto(Exception ex, bool isDev, string traceId)
        {
            ...
            switch (ex)
            {
                case KTMSNotFoundException:
                case KTMSConflictException:
                case KTMSBusinessRuleException:
                    code = "entity.error";
                    message = ex.Message;
                    break;
