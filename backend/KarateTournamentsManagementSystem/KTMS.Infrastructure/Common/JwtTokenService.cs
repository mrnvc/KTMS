...
namespace KTMS.Infrastructure.Common
{
    public sealed class JwtTokenService : IJwtTokenService
    {
        ...
        public JwtTokenPair IssueTokens(KTMSUserEntity user)
        {
            ...
            var claims = new List<Claim>
        {
            ...
            new("is_contestant",  user.IsContestant.ToString().ToLowerInvariant()),
            new("is_coach", user.IsCoach.ToString().ToLowerInvariant()),
            ...
            if (user.IsContestant)
                claims.Add(new Claim(ClaimTypes.Role, "Contestant"));
            if (user.IsCoach)
                claims.Add(new Claim(ClaimTypes.Role, "Coach"));

            // Fallback role if no flags are set
            if (!user.IsAdmin && !user.IsContestant && !user.IsCoach)
